...
with st.expander("ℹ️ About This Dashboard", expanded=True):
    ...
    #### 💡 Features
    - Filter by **purchase category**, **purchase channel**, **gender**, and **income level**
...
# ----- TAB 1: OVERVIEW -----
with tab_overview:
    st.subheader("Revenue by Purchase Category")
...
category_input = st.selectbox("Preferred Purchase Category:", sorted(df["Purchase_Category"].unique()))
...
if st.button("Get My Insights"):
    ...
    top_category = user_segment["Purchase_Category"].mode()[0]
    avg_frequency = user_segment["Frequency_of_Purchase"].mean()

    overall_avg_spending = df["Purchase_Amount"].mean()
    overall_avg_frequency = df["Frequency_of_Purchase"].mean()
